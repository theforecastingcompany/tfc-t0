...
# Rotary position embedding ported from Datadog's Toto RoPE backbone
...
class TimeAwareRotaryEmbedding(RotaryEmbedding):
    ...
    def get_scale(
        ...
        The signature intentionally drops the parent's ``seq_len`` and ``offset`` kwargs
        since this implementation infers those from ``t``.
        """
        assert self.use_xpos
